% !TEX root = ../Thesis.tex
\acresetall
\myChapter[\acs{fe} \threed reconstruction of the pulmonary acinus]{Finite element \threed reconstruction of the pulmonary acinus imaged by synchrotron X-ray tomography}\label{ch:tsuda2008}
...
\begin{figure}[h]%
	...
	\begin{tikzpicture}[,auto,%
		...
		\begin{scope}[every path/.style=line, rounded corners]
			...
			\end{scope}
		\end{tikzpicture}
	\caption[Workflow for \threed reconstruction of the acinus]{Workflow diagram outlining protocol for high-resolution 3-dimensional (\threed) reconstruction of the acinus.}
	\label{fig:workflow}
\end{figure}
...
\begin{figure}[htb]%
	...
	\caption[\acs{srxtm} imaging setup]{Imaging setup. The X-rays are coupled out of the storage ring via a wiggler. The monochromator is then used to select a desired wavelength of the incident X-rays. The sample is placed on a rotating sample holder inside the beam. While the X-rays penetrate the sample, the stage is rotated, and in total 1500 projections over \SI{180}{\degree} are recorded. Plot shows spectrum of \ac{tomcat} (red and blue) compared with conventional X-ray tube (green).}%
...
\begin{description}
	\item[1. Mesh generation] A \ac{fe} mesh (\autoref{subfig:tsuda-04b}), initially uniform, is iso\-trop\-ic\-ally generated over all pixels. We position the nodes of the \ac{fe} mesh at the centers of the existing voxels (\autoref{subfig:tsuda-04b}, \emph{right}). This means that each \ac{fe} overlaps 2$\times$2 voxels in the \twod case. (\autoref{subfig:tsuda-04b}, \emph{right}) or 2$\times$2$\times$2 voxels in the \threed case (\autoref{subfig:tsuda-04b}, \emph{left}). The black circles represent nodes generated inside the object and the white circles denote nodes outside of the object. The nodes inside the object have a pixel or voxel value higher than the chosen threshold and the nodes outside the object have a value lower than the chosen threshold. Each \ac{fe} node is assigned the grayscale value of the corresponding voxel. Note that the boundary between the black and white nodes is not smooth at this stage.
...
\begin{figure}[p]%
	...
	\caption[Overview of workflow for \acs{roi} selection]{Overview of workflow for the selection of a \acf{roi}. \subref{subfig:tsuda-05a}: full perspective view of the sample R108C60C-07e with 6 selected orthogonal slices and bounding box of the full sample in orange. The \ac{roi} selected for subfigures \subref{subfig:tsuda-05c} and \subref{subfig:tsuda-05d} is visible in the \emph{top left}. \subref{subfig:tsuda-05b}: close-up of one slice of the full dataset with selected \ac{roi} (side-length of 256 pixels). \subref{subfig:tsuda-05c}: \ac{roi} cut out of the full sample, 6 selected orthogonal slices are shown. \subref{subfig:tsuda-05d}: close-up of the \ac{roi} in subfigure \subref{subfig:tsuda-05b}.}
	\label{fig:tsuda-05}
\end{figure}%
	...
	\noindent\makebox[\textwidth]{%
		\subfloat[]{%
			...
			\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
				...
				\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{img/Tsuda2008/Tsuda-06a.png}};
...
The surface area density of air space (S\textsubscript{VS} [\si{\centi\metre\squared\per\centi\metre\cubed}]) of each \ac{roi} sample was also calculated and plotted (\autoref{fig:VVSplot}). As we expected, however, S\textsubscript{VS} shows poor match with the reported value of \num[separate-uncertainty=true]{904(84)}~(\acs{se}) over almost the entire range of threshold levels tested and especially at the threshold value of 139. The cause of this discrepancy will be discussed in detail in \autoref{sec:discussion}.
...
\begin{figure}[htb]%
	...
	\caption[\threed reconstruction of tissue]{Three-dimensional reconstruction of tissue. \subref{subfig:tsuda-07a} and \subref{subfig:tsuda-07d}: threshold 130. \subref{subfig:tsuda-07b} and \subref{subfig:tsuda-07e}: threshold 139. \subref{subfig:tsuda-07c} and \subref{subfig:tsuda-07f}: threshold 148.}
	\label{fig:tsuda-07}
\end{figure}%
...
\begin{figure}[htb]%
	...
	\caption[\threed reconstruction of air spaces]{\subref{subfig:tsuda-09a}: reconstruction of \threed air spaces separated by tissues from \ac{roi}. \subref{subfig:tsuda-09b}: enlargement of a small alveolated air section isolated from the center of the \ac{roi}.}
	\label{fig:tsuda-09}
\end{figure}%
...
\begin{figure}[htb]%
	...
	\def\x{940}%
	\def\y{1150}% scalebar-y at 90% of height of y=1715px
	\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
		\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{img/Tsuda2008/Tsuda-10_edit}};
...
% \renewcommand\bibname{Bibliography | Chapter 6}%
