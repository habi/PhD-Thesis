...
Mammalian lung development can be divided into five overlapping stages~\cite{Schittny2004,Schittny2007a}, which are shown in the gray region in \autoref{fig:lung development stages old}%
\todo{\autoref{fig:lung development stages old} shows rat. How is it for humans? Simply other x-axis? $\rightarrow$ bug JCS!}%
...
\begin{figure}[htb]
	...
	\todo[inline]{Does this plot add more information? Or should we just put it below \autoref{fig:lung development stages old} and name them ``Old''/''New''? \autoref{fig:lung development stages old} old model according to \cite{Schittny2007a}, \autoref{fig:lung development stages new} is newly postulated model by Schittny after newest results with \ac{srxtm}.}
\end{figure}
